...
public class LowLevel extends Canvas implements CommandListener {
	...
	public LowLevel( Tycoon main, Display display, int mission, int number, int[] order, String bgm, int timelimit)
	{
		state = 0;
		this.main = main;
		...
		try
		{
			...
			guest			=	Image.createImage("/images/guest0.gif");
			plate			=	Image.createImage("/images/plate.gif");
			enter			=	Image.createImage("/images/order0.gif");
			...
			order_img[0]	=	Image.createImage("/images/level0.gif");
			order_img[1]	=	Image.createImage("/images/level1.gif");
			order_img[2]	=	Image.createImage("/images/level2.gif");
	...
	protected void paint(Graphics g) 
	{
		...
		{// 손님의 주문 요구 - 레벨에 따른 주문 화면 풀력
			if( mission == 0 )		g.drawImage(order_img[0], width/2, height/2, Graphics.HCENTER|Graphics.VCENTER);		
			else if( mission == 1 )	g.drawImage(order_img[1], width/2, height/2, Graphics.HCENTER|Graphics.VCENTER);		
			else if( mission == 2 )	g.drawImage(order_img[2], width/2, height/2, Graphics.HCENTER|Graphics.VCENTER);		
		}
		else{// state == 2;
		    //각 이미지의 크기를 얻음
			// 메인캐릭 60 * 60
			...
			//int bgr_height = bgr.getHeight();
			//int bgr_width = bgr.getWidth();
			...
			Font f=Font.getFont(0, 1, 16 );	//face:SYSTEM(0), style:BOLD(1), size:LARGE(16)
			g.setFont(f);
			...
			// 배경 위에 토핑 그리기 (map에서 각 element의 값을 확인해서 해당하는 그림을 그림)
			for(int i=0; i< row; i++)
			{
				 for(int j=0; j<col; j++)
				 {
					...
					else if (map.mapArray[i][j] == 10 )		//주인공 그림
					{
					   g.drawImage( surprise,60+j*so_width, 72 + i*topping_height, Graphics.TOP|Graphics.LEFT);
					}
	...
	{// 역시 게임 중에만 핸들링.
		...
		else if( c == okCmd  )
		{
			...
			{
				state = 2;
				map = new LowMap(this, mission, timelimit);
			}
